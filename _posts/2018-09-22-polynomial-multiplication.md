...
## Motivation
Many problems can be reduced to a polynomial multiplication.
...
After formalizing the problem, we'll see the Karatsuba algorithm for multiplying polynomials in $O(n^{1.58})$ time.
...
The input consists of two polynomials $a$ and $b$ of the same degree $n$ (if this
is not the case we can always pad with zeroes), and the output is a polynomial $c$ of degree $2n$:
...
One multiplication is replaced by a few additions (which are cheap) so now we need only **three multiplications** of half-degreed polynomials.
...
It is known that $n+1$ pairs $(x_0, y_0), .. , (x_n, y_n)$ where $x_i \neq x_j$ for $i \neq j$ uniquely determine a polynomial $p$ of degree $n$ where $p(x_i) = y_i$. We say that from evaluations of a polynomial in $n+1$ points we can reconstruct (interpolate) the polynomial. Simplest intuition for this is that a polynomial has $n + 1$ unknown coefficients, and each evaluation is an equation.
...
How to find the value of $c$ in a point $x$? As $c(x) = a(x)b(x)$, it is enough to evaluate both polynomials in $x$ and multiply the results.
...
This is great, but already the first step is difficult to do in better than $O(n^2)$ time.
Lucky for us, someone noticed that evaluation of a polynomial in many points (multipoint evaluation) can be done more efficiently if the chosen set of points satisfies a few rules.
Even more, the third step in that case can also be reduced to the multipoint evaluation of a similar set of points.
The second step is clearly linear, so this sounds promising.

Let's see first how to do the multipoint evaluation efficiently.
...
The only thing we are missing is the value of $w$...
...
* $\cdot$ is element-wise multiplication operator on polynomials, that is: $(r \cdot s)_i = r_is_i$.
...
Note that $(g^{(p-1)/N)})^N=1$. So we can use the same transformation algorithm as before,
...
## Conclusion
We started with the polynomial multiplication problem but we also learned how to do FFT efficiently. FFT, on the other hand, is used everywhere (signal processing for example). Some big-integer libraries still use the Karatsuba algorithm, while others have opted for FFT or even fancier algorithms.
